...
gc_get_stats(struct gc_stats *ring, int gen)
{
    if (gen == 0) {
...
static struct gc_generation_stats *
gc_get_prev_stats(struct gc_stats *ring, int gen)
{
    if (gen == 0) {
        struct gc_young_stats_buffer *buffer = &ring->young;
        struct gc_generation_stats *stats = &buffer->items[buffer->index];
...
{
    struct gc_generation_stats *prev_stats = gc_get_prev_stats(ring, gen);
    struct gc_generation_stats *cur_stats = gc_get_stats(ring, gen);
...
gc_collect_main(PyThreadState *tstate, int generation, _PyGC_Reason reason)
{
    ...
    /* This tree's default (GIL) collector maintains no running live-object
       ...
       heap_size is left zero, exactly as the free-threaded build leaves it. */
    ...
    */
    ...
    /* Keep stock's three counters in step.  They are what an extension
       ...
       against stock 3.13 observes a frozen counter. */

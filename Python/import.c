...
#include "pycore_ceval.h"         // _PyEval_GetBuiltins()
#include "pycore_dict.h"          // _PyDict_GetItemKeepLazy()
...
#include "pycore_moduleobject.h"  // PyModule_GetDict()
...
static inline int
should_be_lazy(PyThreadState *tstate, PyObject *name, PyObject *fromlist)
{
    if (tstate->interp->eager_imports != NULL) {
        ...
        if (fromlist != NULL && fromlist != Py_None) {
            ...
            for (Py_ssize_t i = 0; i < size; ++i) {
                ...
                if (found) {
                    return 0; /* If the module is flagged as eager import, load eagerly */
                }
            }
        }
        if (size == 0) {
        ...
        if (parent_module == NULL) {
            ...
            if (PyDict_CheckExact(parent_dict)) {
                ...
                if (res < 0) {
                    ...
                    if (_PyDict_GetItemKeepLazy(parent_dict, child) != NULL) {
                        PyObject *already_loaded = _PyImport_GetModule(tstate, name);
                        Py_XDECREF(already_loaded);
...
static inline int
prepare_lazy_import(PyThreadState *tstate, PyObject *builtins, PyObject *name, PyObject *fromlist)
{
    int lazy = should_be_lazy(tstate, name, fromlist);
    if (lazy == 1) {
        lazy = add_lazy_modules(tstate, builtins, name);
    }
    return lazy;
}
                     ...
                     PyObject *name, PyObject *fromlist, PyObject *level)
{
    ...
    stack[3] = fromlist == NULL ? Py_None : fromlist;
    stack[4] = level;
                         ...
                         PyObject *name, PyObject *fromlist, PyObject *level)
{
    ...
    int lazy = prepare_lazy_import(tstate, builtins, abs_name, fromlist);
    if (lazy < 0) {
        goto error;
    }
    if (lazy) {
...
PyObject *
PyImport_SetLazyImports(PyObject *enabled, PyObject *eager)
{
...
The optional argument `eager` can be any container of strings; all imports
whose full name is present in the container will be eager.
[clinic start generated code]*/

static PyObject *
_imp__set_lazy_imports_impl(PyObject *module, PyObject *enabled,
                            PyObject *eager)
/*[clinic end generated code: output=4fddb3389e4d066b input=9133a407291cd3ae]*/
{
    return PyImport_SetLazyImports(enabled, eager);
}

int
_PyImport_IsLazyImportsEnabledTstate(PyThreadState *tstate)
{
...
{
    PyThreadState *tstate = _PyThreadState_GET();
    return _PyImport_IsLazyImportsEnabledTstate(tstate);
}

...
#define LAZY_MODULES(interp) \
    _PyImport_GetLazyState(interp)->modules
#define LAZY_PENDING_SUBMODULES(interp) \
    _PyImport_GetLazyState(interp)->pending_submodules
...
#define LAZY_IMPORT_FUNC(interp) \
    _PyImport_GetLazyState(interp)->import_func
...
#define LAZY_IMPORTS_MODE(interp) \
    _PyImport_GetLazyState(interp)->mode

#define LAZY_IMPORTS_FILTER(interp) \
    _PyImport_GetLazyState(interp)->filter

#ifdef Py_GIL_DISABLED
#define LAZY_IMPORTS_LOCK(interp) \
    PyMutex_Lock(&_PyImport_GetLazyState(interp)->mutex)
#define LAZY_IMPORTS_UNLOCK(interp) \
    PyMutex_Unlock(&_PyImport_GetLazyState(interp)->mutex)
#else
#define LAZY_IMPORTS_LOCK(interp)
#define LAZY_IMPORTS_UNLOCK(interp)
#endif
...
{
    assert(interp->_malloced == NULL);

    _PyLazyImportsState *state = PyMem_Calloc(1, sizeof(*state));
    if (state == NULL) {
        PyErr_NoMemory();
        return NULL;
    }
    interp->_malloced = state;

    state->builtins_import_name = PyUnicode_InternFromString("__lazy_import__");
    state->module_list_name = PyUnicode_InternFromString("__lazy_modules__");
    state->find_and_load_submodule_name =
        PyUnicode_InternFromString("_find_and_load_lazy_submodule");
    state->ast_is_lazy_name = PyUnicode_InternFromString("is_lazy");
    state->pending_submodules = PyDict_New();
    state->modules = PySet_New(NULL);
    if (state->builtins_import_name == NULL ||
        state->module_list_name == NULL ||
        state->find_and_load_submodule_name == NULL ||
        state->ast_is_lazy_name == NULL ||
        state->pending_submodules == NULL ||
        state->modules == NULL)
    {
        Py_CLEAR(state->builtins_import_name);
        Py_CLEAR(state->module_list_name);
        Py_CLEAR(state->find_and_load_submodule_name);
        Py_CLEAR(state->ast_is_lazy_name);
        Py_CLEAR(state->pending_submodules);
        Py_CLEAR(state->modules);
        PyMem_Free(state);
        interp->_malloced = NULL;
        return NULL;
    }

    PyStatus status = _PyImport_ConfigureLazyImports(
        _PyInterpreterState_GetConfig(interp));
    if (_PyStatus_EXCEPTION(status)) {
        PyErr_SetString(PyExc_ValueError, status.err_msg);
        Py_CLEAR(state->builtins_import_name);
        Py_CLEAR(state->module_list_name);
        Py_CLEAR(state->find_and_load_submodule_name);
        Py_CLEAR(state->ast_is_lazy_name);
...
PyStatus
_PyImport_ConfigureLazyImports(const PyConfig *config)
{
    PyImport_LazyImportsMode mode = PyImport_LAZY_NORMAL;

    const char *env = config->use_environment
        ? Py_GETENV("PYTHON_LAZY_IMPORTS")
        : NULL;
    if (env != NULL) {
        if (strcmp(env, "all") == 0) {
    ...
    const wchar_t *xoption = _Py_get_xoption(
        &config->xoptions, L"lazy_imports");
    if (xoption != NULL) {
        const wchar_t *separator = wcschr(xoption, L'=');
        const wchar_t *value = separator != NULL ? separator + 1 : L"";
        if (wcscmp(value, L"all") == 0) {
            mode = PyImport_LAZY_ALL;
        }
        else if (wcscmp(value, L"normal") == 0) {
    ...
    _PyImport_GetLazyState(_PyInterpreterState_GET())->startup_mode = mode;
    return _PyStatus_OK();
}

void
_PyImport_ApplyLazyImportsConfig(void)
{
    _PyLazyImportsState *state = _PyImport_GetLazyState(
        _PyInterpreterState_GET());
    state->mode = state->startup_mode;
}
...
_PyImport_LoadLazyImportTstate(PyThreadState *tstate, PyObject *lazy_import)
{
    ...
    _PyLazyImportsState *state = _PyImport_GetLazyState(interp);
    PyObject *importing = state->importing_modules;
    if (importing == NULL) {
        importing = state->importing_modules = PySet_New(NULL);
        if (importing == NULL) {
            _PyImport_ReleaseLock(interp);
            return NULL;
        }
                               ...
                               PyObject **result)
{
    ...
    *result = NULL;
    ...
    PyObject *mod = import_find_and_load_with_name(
        tstate, full_name,
        _PyImport_GetLazyState(tstate->interp)->find_and_load_submodule_name,
        Py_None);
    if (mod == NULL) {
        Py_DECREF(pending_set);
        Py_DECREF(full_name);
...
_PyImport_ClearCore(PyInterpreterState *interp)
{
    ...
    _PyLazyImportsState *state = _PyImport_GetLazyState(interp);
    if (state != NULL) {
        Py_CLEAR(state->import_func);
        Py_CLEAR(state->pending_submodules);
        Py_CLEAR(state->modules);
        Py_CLEAR(state->importing_modules);
        Py_CLEAR(state->filter);
        Py_CLEAR(state->builtins_import_name);
        Py_CLEAR(state->module_list_name);
        Py_CLEAR(state->find_and_load_submodule_name);
        Py_CLEAR(state->ast_is_lazy_name);

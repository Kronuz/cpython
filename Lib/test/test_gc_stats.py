...
class TestGetGCStats(unittest.TestCase):
    ...
    @requires_gil_enabled("heap_size is not maintained in free-threaded builds")
    def test_heap_size_tracks_live_objects(self):
        stats = _remote_debugging.get_gc_stats(
            os.getpid(), all_interpreters=False)
        item = get_last_item(stats, 0, 0)
        self.assertIsNotNone(item)
        self.assertGreater(item.heap_size, 0)
